package com.uimirror.location.core.coordinate;
...
public final class GeographicCoordinate implements Serializable {
    ...
    private GeographicCoordinate(BigDecimal value, GeographicCoordinateType geographicCoordinateType) {
        ...
        if (geographicCoordinateType == GeographicCoordinateType.LATITUDE) {
            cardinalDirection = (value.compareTo(BigDecimal.ZERO) == 0) ? CardinalDirection.ZERO_LATITUDE_DEGREE : (value.compareTo(BigDecimal.ZERO) > 0) ? CardinalDirection.NORTH : CardinalDirection.SOUTH;
        } else {
            cardinalDirection = (value.compareTo(BigDecimal.ZERO) == 0) ? CardinalDirection.ZERO_LONGITUDE_DEGREE : (value.compareTo(BigDecimal.ZERO) > 0) ? CardinalDirection.EAST : CardinalDirection.WEST;
        }
    }
    ...
    private GeographicCoordinate(int degrees, int minutes, int seconds, int millis, CardinalDirection cardinalDirection) {
        ...
        this.value = BigDecimal.valueOf(degrees)
                ...
                .multiply(BigDecimal.valueOf(((cardinalDirection == CardinalDirection.ZERO_LATITUDE_DEGREE) || (cardinalDirection == CardinalDirection.ZERO_LONGITUDE_DEGREE) || (cardinalDirection == CardinalDirection.NORTH) || (cardinalDirection == CardinalDirection.EAST)) ? 1 : -1));
    }
