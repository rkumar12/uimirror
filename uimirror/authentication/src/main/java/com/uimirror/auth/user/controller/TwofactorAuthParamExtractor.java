...
import com.uimirror.auth.user.bean.TwofactorAuthentication;
 ...
 * using {@link TwofactorAuthentication}
 * 
 * {@link TwofactorAuthParamExtractor#extractAuthParam(BasicAuthenticationForm)} will extract the
 * {@link Authentication} object performing {@link ValidatorService#validate(Object)}
 * 
 * @author Jay
 */
public class TwofactorAuthParamExtractor extends DefaultAuthParamextractor{
	
	private static final Logger LOG = LoggerFactory.getLogger(TwofactorAuthParamExtractor.class);
	...
	public Authentication extractAuthParam(BasicAuthenticationForm param) {
		LOG.info("[START]- Extracting the two Factor form authentication parameter");
		Authentication auth = null;
		if(isValid(param))
			auth = getOTPFormDetails(param);
		LOG.info("[END]- Extracting the two Factor form authentication parameter");
		return auth;
	}
	...
	public boolean validate(Object src) {
		...
		if(!valid && !CredentialType._2FA.equals(param.getCredentialType()))
			valid =  Boolean.FALSE;
	 ...
	 */
	private Authentication getOTPFormDetails(final BasicAuthenticationForm param){
		...
		return new TwofactorAuthentication(param.getAccessToken(), param.getTokenEncryptStartegy(), 
