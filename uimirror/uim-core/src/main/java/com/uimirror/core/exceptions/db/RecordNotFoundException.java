/*******************************************************************************
 * Copyright (c) 2014 Uimirror.
 * All rights reserved. This program and the accompanying materials
 * are made available under the terms of the Uimirror license
 * which accompanies this distribution, and is available at
 * http://www.uimirror.com/legal
 *
 * Contributors:
 * Uimirror Team
 *******************************************************************************/
package com.uimirror.core.exceptions.db;

import com.uimirror.core.ErrorCodes;


/**
 * When a record/document not found for the given search criteria, then default 
 * error code will be {@link ErrorCodes#_404}
 * @author Jay
 */
public class RecordNotFoundException extends DBException{

	private static final long serialVersionUID = 5952640506213878692L;
	
	public RecordNotFoundException() {
		super(ErrorCodes._404, "Document Not found with given creteria");
	}

	/**
     * Constructs an {@code RecordNotFoundException} with the specified message and root cause.
     *
     * @param msg the detail message
     */
    public RecordNotFoundException(String msg) {
        super(ErrorCodes._404, msg);
    }

}
